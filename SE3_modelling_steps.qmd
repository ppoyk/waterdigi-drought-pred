...
```{r, join-further-lags}
# Take the best correlation accum + lag values, and join to them more lagged values
# Compare the differences. How much would we lose correlation if using larger lag?
pipe_cor_smry[pipe_cor[lag_months==0]]

pipe_cor_smry[ind_accum==24]

SEI::plot_sei(station_spi_spei_ts[station_id == "0302" & dr_ind == "SPEI24"]$value)
station_lag_inds[,unique(station_id)]
```
...
Miten eri indeksien mukaan Suomessa on ollut kuivuutta? Myös alueellinen tarkastelu.
Kysy Laurilta.
