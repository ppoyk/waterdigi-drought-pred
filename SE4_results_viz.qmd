...
author:
  - name:
     ...
     family: "Pöykkö"
    degrees: "MSc (tech)"
    orcid: "0009-0000-1214-5336"
    corresponding: true
    email: pietari.poykko@oulu.fi
    roles:
      - Investigation
...
affiliations:
  - id: ou
    ...
    department: "Water, Energy and Environmental Engineering Research Unit (WE3)"
    address: "Pentti Kaiteran katu 1, Linnanmaa"
    city: "Oulu"
    region: "North Ostrobothnia"
    country: "Finland"
    postal-code: "FI-90014"
...
The figures were not saved into files outside of this document. Although easily accomplished, this was omitted to keep the code compact.
...
However, the approach taken brings some convenience. All object classes, factor levels and their order etc. are now preserved. Saving objects as `csv`s would require writing wrapper functions for file-reading to ensure consistent object attributes/metadata. These intermediary data files need to only be valid for the duration of the project rendering, so "universal" storage is not necessary. Some heavy object calculation results were saved also on disk for more rapid code development. By the end of making this document I learned that perhaps this is not ideal, as loading and saving the environment every time you want to render the file gets quite slow. And getting a notebook render to visually look just the way you like it *takes A LOT of tweaking*!!! The rendering time could be reduced if only the absolute necessary objects were loaded from their specific files.
...
prob_station <- "0902"
SEI::plot_sei(station_spi_spei_ts[station_id == prob_station & dr_ind == "SPEI6"]$value)
...
:::: {#fig-maps .column-page layout="[3,8]"}
::: {#fig-stations-map}
```{r}
