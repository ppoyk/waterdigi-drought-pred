...
## Modelling framework:
The modelling is done entirely using the freely available R-language (v.4.5.0).
The reproducibility of this workflow is ensure with `renv`.
Libraries should be handled automatically, and all required are listed in `renv.lock`
...
Erroneys (< -1000) SPI & SPEI index values are removed. No ohter outliers.
...
SGI series are then correlated with SPI & SPEI time series.
**The interpretation and visualization are still in progress. The correlation might also get more sophisticated.**

## How to use this notebook:
This notebook can be run with Quarto installed. The easiest way to achieve this is to use the Positron IDE.
The notebook SE1_data_access.qmd should be run in the same env/session than this. Variables defined/loaded there are used here.
This can be achieved using the reproducibility wrapper (`run_reproducibility.R`)
